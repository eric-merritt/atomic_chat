...
class ReadTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "end_line": {
                "type": "integer",
                "description": "Last line to include (exclusive). Capped at start_line+200.",
            },
        },
        "required": ["path"],
    }
    ...
    def call(self, params: str, **kwargs) -> dict:
        ...
        numbered = [
            f"{i:>6}  {line.rstrip()}"
            for i, line in enumerate(subset, start=start_line + 1)
        ]
            ...
            if match:
                ...
                for j in range(i, min(i + 10, len(lines))):
                    if lines[j].strip().startswith('"""') or lines[
                        ...
                        if (
                            "description" in docstring.lower()
                            or "purpose" in docstring.lower()
                        ):
                            purpose = docstring
                            break
                structure.append(
...
class WriteTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "chunk_size": {
                "type": "integer",
                "description": "Maximum tokens per chunk. Default: 4096.",
            },
        },
...
class GrepTool(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> dict:
        ...
        try:
            ...
            if result.returncode != 0:
                return tool_result(
                    error=f"Ripgrep search error: {result.stderr.strip()}"
                )
...
class FindDefinitionTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "def_type": {
                "type": "string",
                "description": 'Type of definition: "function", "class", or "any". Default: "any".',
    ...
    def call(self, params: str, **kwargs) -> dict:
        ...
        if not results:
            return tool_result(
                error=f'No definition found for "{name}" in {os.path.abspath(path)}'
            )
...
class ReplaceTool(BaseTool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "end_line": {
                "type": "integer",
                "description": "Last line to replace (1-indexed, inclusive). -1 = to end of file.",
            },
            "replacement": {
                "type": "string",
                "description": "Content to replace the specified lines with.",
            },
        },
    ...
    def call(self, params: str, **kwargs) -> dict:
        ...
        end_idx = (
            n if end_line == -1 else end_line
        )  # end_line is inclusive, slice end is exclusive

        if start_idx < 0 or start_idx >= n:
            return tool_result(
                error=f"Invalid start_line: {start_line}. File has {n} lines."
            )
        if end_idx < start_line or end_idx > n:
            return tool_result(
                error=f"Invalid end_line: {end_line}. Must be >= start_line ({start_line}) and <= {n}."
            )
